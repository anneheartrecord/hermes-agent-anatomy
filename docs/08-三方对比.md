...
Hermes Agent 是 Nous Research 做的开源全栈 Agent，33 万行 Python。Claude Code 是 Anthropic 的官方编程助手，51 万行 TypeScript，源码因 npm sourcemap 泄露。OpenClaw 是OpenClaw 平台，100 万行+，基于插件体系服务 80+ 渠道。
...
**OpenClaw** 是另一个极端。它不是给个人用户的，是给企业内部团队的。一个 Agent 实例跑在 Docker 容器里，通过 Gateway 同时服务微信、企微、Slack、飞书等 80+ 个渠道的用户。追求的是**稳定性、可管理性、多租户隔离**，不需要酷炫的 CLI 交互。
