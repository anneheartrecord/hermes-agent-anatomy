...
Hermes Agent 是 Nous Research 做的开源全栈 Agent，33 万行 Python。Claude Code 是 Anthropic 的官方编程助手，51 万行 TypeScript，源码因 npm sourcemap 泄露。OpenClaw 是我们内部搭建的 TME-Claw 平台，100 万行+，基于插件体系服务 80+ 渠道。
...
| 维度 | Hermes Agent | OpenClaw (TME-Claw) | Claude Code |
|------|-------------|---------------------|-------------|
| 出品方 | Nous Research（开源模型团队） | 内部平台团队 | Anthropic（模型公司） |
| 核心动机 | 用 Agent 数据训练更好的模型 | 用 Agent 服务业务场景 | 用 Agent 释放自家模型能力 |
| 用户画像 | 研究者 + 开源开发者 | 企业内部运维 + 业务团队 | 个人开发者 |
| 开源状态 | 完全开源（Apache 2.0） | 内部平台 | 客户端源码泄露 |
| 代码语言 | Python | TypeScript + Python | TypeScript |
...
**OpenClaw (TME-Claw)** 是另一个极端。它不是给个人用户的，是给企业内部团队的。一个 Agent 实例跑在 Docker 容器里，通过 Gateway 同时服务微信、企微、Slack、飞书等 80+ 个渠道的用户。追求的是**稳定性、可管理性、多租户隔离**，不需要酷炫的 CLI 交互。
...
| 维度 | Hermes Agent | OpenClaw (TME-Claw) | Claude Code |
|------|-------------|---------------------|-------------|
| 语言 | Python | TypeScript + Python 后端 | TypeScript |
| 运行时 | CPython 3.10+ | Node.js + Docker | Bun 1.3.11+ |
| 架构风格 | **单体**（run_agent.py 9,431 行） | **插件体系**（Extensions + Gateway） | **模块化单仓**（2,766 个文件） |
| 入口数量 | 2（CLI + Gateway） | 3+（Gateway + API + 管理后台） | 1（CLI） |
| 构建工具 | pip / uv | Webpack + Docker | Bun bundler |
...
OpenClaw 走了另一条路：**插件化**。核心是一个轻量的 Gateway 进程，所有能力通过 Extensions 挂载。IM 渠道是插件，工具是插件，存储后端是插件。单个实例跑在 Docker 容器里，通过 Kubernetes 做编排和灰度。Django 后端的 `devops_gray_api_view.py` 管实例生命周期，`openclaw_api.py` 管 Gateway RPC 通信。
...
│                │        │                │         ├────────────────┤
│  全部共用      │        │  Django 管理    │         │  permissions/  │
│  一个类        │        │  Docker 隔离    │         │  6,300 行      │
└────────────────┘        └────────────────┘         │  compact/      │
                                                     │  26 个文件     │
                                                     └────────────────┘
...
OpenClaw 的做法完全相反：**每个 Agent 实例只绑一个 Provider。** 凭证通过 `entrypoint.sh` 在容器启动时注入。Provider 切换是运维操作，不是用户操作。简单，但够用。
...
| 多租户 | 单实例多用户（通过 user_id 隔离） | 多实例（Docker 容器级隔离） | 单用户 |
...
OpenClaw 用 **QMD 格式**做知识索引，相对轻量。核心文档和配置通过 QMD 文件管理，Agent 可以按需检索。适合企业内部已有知识库的场景。
...
| 权限模式 | 工具启用/禁用 | 运维管控（容器级隔离） | **三模式**（default / auto / plan） |
...
**OpenClaw** 没有内置权限系统，安全依赖容器级隔离和运维管控。每个 Agent 实例跑在独立的 Docker 容器里，容器的 namespace 天然限制了它能看到的文件系统和网络。灰度部署由 Django 后端控制。这对企业内部平台来说足够了。
...
| 维度 | Hermes Agent | OpenClaw (TME-Claw) | Claude Code |
|------|-------------|---------------------|-------------|
| 安装方式 | `pip install` / `uv install` | Docker 镜像 + K8s 编排 | `npm install -g @anthropic-ai/claude-code` |
| 运行环境 | 本地 / Docker / Modal / SSH / Daytona / Singularity | Docker 容器（CentOS/Tlinux 基础镜像） | 本地终端（Bun 运行时） |
| 多实例 | Gateway 单进程多用户 | K8s Pod 级别水平扩展 | 单用户单实例 |
| 配置管理 | `cli-config.yaml` + 环境变量 | `entrypoint.sh` 注入 + Django 管理后台 | `settings.json` + CLAUDE.md |
| 灰度能力 | 无内置 | `devops_gray_api_view.py` 灰度部署 | 无（82 个 feature flag 做能力灰度） |
| 运维 | 用户自运维 | 平台团队运维 | Anthropic 托管 API + 用户本地 CLI |
| 资源隔离 | 6 种终端后端选择 | Docker namespace | 文件系统沙箱 |
...
OpenClaw 是**典型的企业级容器化部署**。每个 Agent 实例是一个 Docker 容器，通过 Kubernetes 做编排、扩缩容和灰度。`entrypoint.sh` 负责容器启动时的配置注入（Provider 凭证、模型参数、插件列表）。Django 后端提供管理界面和灰度部署 API。
...
Claude Code 和 OpenClaw 没有做这个，原因不同。Claude Code 背后是 Anthropic，训练数据的采集和处理在服务端完成，不需要在客户端暴露。OpenClaw 是内部平台，模型是腾讯混元，训练流程在模型团队那边。
...
| 指标 | Hermes Agent | OpenClaw (TME-Claw) | Claude Code |
|------|-------------|---------------------|-------------|
...
OpenClaw 的 100 万行+ 包含了前端（Vue.js）、后端（Django）、微信插件、企微渠道等多个子项目的总量。单看 Agent 核心，体量小得多。
...
│   │
│   ├─ 需要灰度部署 + 多租户隔离？
│   │   └─→ OpenClaw (K8s + Docker + Django 管理后台)
│   │
│   └─ 预算有限，先跑起来再说？
│       └─→ Hermes Agent (pip install + Gateway 模式)
│
├─ 个人开发者日常编程
│   └─→ Claude Code
