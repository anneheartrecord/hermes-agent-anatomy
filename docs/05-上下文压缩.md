...
OpenClaw 里用的方案比较简单：**固定消息数裁剪 + 手动注入 system reminder**。没有 LLM 摘要，没有结构化模板，直接砍到最近 N 轮，然后在 system prompt 里追加一段提醒告诉模型之前的对话已被截断。
...
实际上在 OpenClaw 的下一个版本里，我们正在考虑引入类似 Hermes 的结构化摘要。不需要做得这么复杂，只要在裁剪前用小模型生成一段简短的 Goal + Progress 摘要注入到 system prompt，就能显著改善长对话的连贯性。
